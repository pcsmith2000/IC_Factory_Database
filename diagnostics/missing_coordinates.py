...
def run():
    with connection() as db:
        ...
        rows=db.execute("SELECT g.facility_key AS facility_id,g.release_tag,g.name,g.address,g.city,g.state,g.zip,g.address__source,d.tier FROM golden_facility g LEFT JOIN dim_facility d ON g.facility_key=d.facility_key WHERE NULLIF(btrim(g.lat_lon),'') IS NULL ORDER BY g.facility_key").fetchall()
        ...
        cache_rows=db.execute("SELECT cache_key,kind,result,found,provider,fetched_at FROM cache_lookup WHERE cache_key=ANY(%s) OR kind='places_state'",(sorted(keys),)).fetchall()
        events=[]
    ...
    for g in rows:
        ...
        if not d['has_address']:
            ...
            else:flags['address_rows_places_state_not_done']+=1
    ...
    summary={**totals,'queried_at':when,'by_diagnosis':dict(counts),'selection_flags':dict(flags),'geocode_accuracy_for_address_rows':dict(acc),
             ...
             'by_state':{k:dict(v) for k,v in states.items()},'by_tier':dict(tiers),'current_sources_facility_counts':dict(sources),
