...
Subsequent rounds use four independent row workers within each 100-row batch; the selected cohort and novelty accounting remain unchanged. `coverage_complete` checks the actual unique result IDs against all 241 frozen IDs, not only a success flag or summed job counts.
